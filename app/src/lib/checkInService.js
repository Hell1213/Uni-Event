...
import { validate, validateAndSetDoc, validateAndUpdateDoc, checkInSchema, eventUpdateSchema, participantSchema, ticketSchema } from './validators';
...
const syncOfflineCheckInItem = async (item, eventId, organizerId) => {
    ...
    if (!checkInSnap.exists()) {
        ...
        await validateAndSetDoc(checkInRef, {
            userId: item.userId,
        ...
        if (item.ticketId) {
            await validateAndUpdateDoc(doc(db, 'tickets', item.ticketId), {
                checkInStatus: 'checked-in',
                checkedInAt: offlineCheckedInAt,
                checkedInBy: organizerId,
            }, ticketSchema).catch(() => {});
        } else {
            await validateAndUpdateDoc(doc(db, 'events', eventId, 'participants', item.userId), {
                checkInStatus: 'checked-in',
                checkedInAt: offlineCheckedInAt,
                checkedInBy: organizerId,
            }, participantSchema).catch(() => {});
        }

        await validateAndUpdateDoc(doc(db, 'events', eventId), {
            'stats.totalCheckedIn': increment(1),
            'stats.lastCheckInAt': serverTimestamp(),
        }, eventUpdateSchema).catch(() => {});

        const registrationRef = doc(db, 'events', eventId, 'registrations', item.userId);
        await validateAndUpdateDoc(registrationRef, { status: 'attended' }, participantSchema).catch(() => {});
    }
};
