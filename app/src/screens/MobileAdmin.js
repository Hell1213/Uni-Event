...
import { validateAndUpdateDoc, eventUpdateSchema, clubUpdateSchema, userUpdateSchema } from '../lib/validators';
...
export default function MobileAdmin() {
    ...
    const handleConfirmSuspend = async () => {
        ...
        try {
            await validateAndUpdateDoc(doc(db, 'events', targetEventId), {
                status: 'suspended',
                appealStatus: 'none',
    ...
    const handleApproveClub = async (reqId, ownerId) => {
        try {
            await validateAndUpdateDoc(doc(db, 'clubs', reqId), { approvalStatus: 'approved' }, clubUpdateSchema);
            if (ownerId) {
                await validateAndUpdateDoc(doc(db, 'users', ownerId), { role: 'club', isVerified: true }, userUpdateSchema);
    ...
    const handleRejectClub = async reqId => {
        try {
            await validateAndUpdateDoc(doc(db, 'clubs', reqId), { approvalStatus: 'rejected' }, clubUpdateSchema);
